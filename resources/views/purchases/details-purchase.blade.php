                    ...
                    <div class="card-body">
                        ...
                        <div class="row gx-3 mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-control form-control-solid">{{ $purchase->paid }}</div>
                            </div>
                            <div class="col-md-6">
                                <label class="small mb-1">Total Bayar</label>
                                <div class="form-control form-control-solid">{{ $purchase->total_amount_paid }}</div>
                            </div>
                        </div>
                        <!-- Form Row -->
                        <div class="row gx-3 mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <div
                                    class="form-control form-control-solid">{{ $purchase->user_updated ? $purchase->user_updated->name : '-' }}</div>
                            </div>
                        </div>
                        <!-- Form Group (address) -->
                        <div class="mb-3">
                        ...
                        @elseif($purchase->paid !== 0)
                            <form action="{{ route('purchases.updatePurchasePaid') }}" method="POST">
                                @csrf
                                @method('put')
                                <input type="hidden" name="id" value="{{ $purchase->id }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="amount_paid" class="form-control form-control-solid my-4" required>
                                </div>

                                <!-- Submit button -->
                                <button type="submit" class="btn btn-warning my-2">
                                    Setujui Pembayaran Pembelian
                                </button>
                            </form>

                            <a class="btn btn-primary" href="{{ URL::previous() }}">Back</a>
                            <a class="btn btn-danger"
                               href="{{ route('purchases.createReturn', $purchase->id) }}">Retur</a>
                        @else
                        ...
                        @if($purchase->arrival_date != null)
                            <a class="btn btn-success" href="{{ route('purchases.invoiceBill', $purchase->id) }}">Tagihan</a>
                        @endif
                    </div>
                </div>
            </div>
                                ...
                                <table class="table table-striped align-middle">
                                    ...
                                    @foreach ($purchaseDetails as $item)
                                        <tr>
                                            ...
                                            <td scope="row"><span class="btn btn-success">{{ $item->quantity }}</span>
                                            </td>
